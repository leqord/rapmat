...
class TestModalDialog:
    ...
    def test_confirm_esc_emits_close_signal(self):
        from rapmat.tui.widgets.dialog import ModalDialog

        signals = []
        parent = urwid.SolidFill("x")
        dlg = ModalDialog.confirm("T", "msg", parent, on_close=lambda ok: None)
        urwid.connect_signal(dlg, "close", lambda w, ok: signals.append(ok))
        dlg.keypress((80, 24), "esc")
        assert signals == [False]

    def test_info_esc_calls_on_close(self):
        from rapmat.tui.widgets.dialog import ModalDialog

        received = []
        parent = urwid.SolidFill("x")
        dlg = ModalDialog.info(
            "T", "msg", parent, on_close=lambda: received.append(True)
        )
        dlg.keypress((80, 24), "esc")
        assert received == [True]

    def test_error_esc_runs_last_action_by_default(self):
        from rapmat.tui.widgets.dialog import ModalDialog
    ...
    def test_error_esc_respects_action_index(self):
        from rapmat.tui.widgets.dialog import ModalDialog
    ...
    def test_input_text_esc_calls_on_cancel(self):
        from rapmat.tui.widgets.dialog import ModalDialog
    ...
    def test_input_text_typing_passes_through(self):
        from rapmat.tui.widgets.dialog import ModalDialog
...
class TestSearchEdits:
    def test_submit_search_enter_submits_and_exits(self):
        from rapmat.tui.widgets.search import SubmitSearchEdit

        searches, exits = [], []
        edit = SubmitSearchEdit(searches.append, lambda: exits.append(True))
        edit.keypress((40,), "f")
        edit.keypress((40,), "e")
        edit.keypress((40,), "enter")
        assert searches == ["fe"]
        assert exits == [True]

    def test_submit_search_esc_exits_without_search(self):
        from rapmat.tui.widgets.search import SubmitSearchEdit

        searches, exits = [], []
        edit = SubmitSearchEdit(searches.append, lambda: exits.append(True))
        edit.keypress((40,), "x")
        edit.keypress((40,), "esc")
        assert searches == []
        assert exits == [True]

    def test_live_search_fires_on_every_keystroke(self):
        from rapmat.tui.widgets.search import LiveSearchEdit

        changes, exits = [], []
        edit = LiveSearchEdit(changes.append, lambda: exits.append(True))
        edit.keypress((40,), "a")
        edit.keypress((40,), "l")
        assert changes == ["a", "al"]
        edit.keypress((40,), "esc")
...
class TestStatusBar:
    def test_set_hints(self):
        ...
        bar = StatusBar()
        bar.set_hints([("q", "Quit", 10), ("h", "Help", 20)])
...
class TestScreenBuildSmoke:
    ...
    def _make_env(self, tmp_path):
        ...
        store.get_url.return_value = "ws://localhost:8000/rpc"
    ...
    def test_study_detail_screen(self, tmp_path):
        ...
        from rapmat.core.entities import Study

        state.store.get_study.return_value = Study(
            study_id="test-study",
            system="Al-Cu",
            domain="bulk",
            calculator="mock",
    ...
    def _make_mem_env(self):
        ...
        state = AppState(store=store)
        frame = urwid.Frame(body=urwid.SolidFill(" "))
        router = ScreenRouter(frame, urwid.Text(" Rapmat TUI"))
        return state, router

    def test_results_screen(self):
        state, router = self._make_mem_env()
        state.store.create_study("smoke", "Si", "bulk", "mock")
        state.store.create_run(name="smoke-run", study_id="smoke")
    ...
    def test_results_zero_thickness_is_shown(self):
        ...
        state, router = self._make_mem_env()
        state.store.create_study("flat", "AlO", "monolayer", "mock")
        state.store.create_run(
            name="flat-run", study_id="flat", config={"domain": "monolayer"}
        )

        flat = Atoms(
            "Al2O",
            positions=[[0.0, 0.0, 5.0], [2.0, 0.0, 5.0], [1.0, 1.5, 5.0]],
            cell=[4.0, 3.0, 10.0],
            pbc=True,
        )
        add_relaxed_structure(state.store, "flat-run", flat, -5.0, "flat-run/1")
        state.active_run = "flat-run"
        ...
        assert s._results[0].thickness == 0.0
        assert s._show_thickness is True
    ...
    def test_apply_to_db_paints_modal_before_work(self):
        ...
        state, router = self._make_mem_env()
        state.store.create_study("st", "Si", "bulk", "mock")
        state.store.create_run(name="r", study_id="st")
        ids = [f"r/{i}" for i in range(6)]
        state.store.add_generation_placeholders("r", [(i, 1, 1) for i in ids])
        ...
        class _FakeLoop:
            ...
            def draw_screen(self):
                drawn_at.append(s._apply_bar.current if s._apply_bar else None)
        ...
        sim = DedupSimulationResult(
            total=6, kept=3, final_dropped=3, kept_ids=ids[:3], dropped_ids=ids[3:]
        )
        d = dict(
    ...
    def test_phase_analysis_screen(self):
        ...
        s = PhaseAnalysisScreen(state, router)
        w = s.build()
        assert isinstance(w, urwid.Widget)
        assert s._main_frame is not None

    def test_save_naming_and_subdir(self):
        ...
        state, router = self._make_mem_env()
        row = ResultRow(
            structure=Structure(id="alcu-on/3", final_atoms=bulk("Cu", "fcc", a=3.6)),
            index=7,
            run_name="alcu-on",
        )

        res = ResultsScreen(state, router)
        res._run_name = "myrun"
        assert res._save_subdir() == "myrun"
        assert res._save_ident(row) == "alcu-on_7"

        hull = PhaseAnalysisScreen(state, router)
        hull._study_id = "my-study"
        assert hull._save_subdir() == "my-study"
        assert hull._save_ident(row) == "my-study_7_alcu-on/3"
